...
export function ProductEditModal({ product, isOpen, onClose, onSave }: {
  ...
  const handleSave = async () => {
    ...
    const result = await onSave(product.sku, {
      sku: isAdmin ? sku.trim() : product.sku,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-200 bg-slate-50">
          <h2 className="text-xl font-bold text-slate-800">Sửa hàng hóa</h2>
              ...
              <div className="flex gap-4">
                <div className="flex-1 space-y-1.5">
                  <label className="text-xs font-semibold text-slate-600">
                    Mã hàng {!isAdmin && <span className="text-slate-400 font-normal">(chỉ Admin được sửa)</span>}
                  </label>
                  <input
                    type="text"
                    value={sku}
                    ...
                    className={`w-full px-3 py-2 border rounded-md text-sm font-medium ${isAdmin ? "border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" : "border-slate-200 bg-slate-100 text-slate-500"}`}
                  />
                </div>
                <div className="flex-1 space-y-1.5">
                  <label className="text-xs font-semibold text-slate-600">
                    Tồn kho {!isAdmin && <span className="text-slate-400 font-normal">(chỉ Admin được sửa)</span>}
                  </label>
                  <input
