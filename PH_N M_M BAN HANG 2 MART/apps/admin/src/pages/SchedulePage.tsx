...
export function SchedulePage() {
  ...
  useEffect(() => {
    ...
    if (!raw || JSON.parse(raw).length === 0) {
      // Seed initial mock assignments matching the screenshots
      const initialRaw: Omit<ScheduleAssignment, "dateKey">[] = [
        // Ca chiều 3
        { id: "1", shiftId: "ca-chieu-3", date: 20, employeeCode: "NV000037", employeeName: "Lan Nhi", employeeDept: "CS1" },
        { id: "2", shiftId: "ca-chieu-3", date: 21, employeeCode: "NV000037", employeeName: "Lan Nhi", employeeDept: "CS1" },
        { id: "3", shiftId: "ca-chieu-3", date: 22, employeeCode: "NV000036", employeeName: "Lệ Quyên", employeeDept: "CS2" },
        { id: "4", shiftId: "ca-chieu-3", date: 23, employeeCode: "NV000051", employeeName: "Nguyễn Uyên", employeeDept: "CS1" },
        { id: "5", shiftId: "ca-chieu-3", date: 24, employeeCode: "NV000023", employeeName: "Nguyễn Hữu Quốc Anh", employeeDept: "CS2" },
        // Ca tối 3
        { id: "6", shiftId: "ca-toi-3", date: 20, employeeCode: "NV001", employeeName: "Thanh Tâm", employeeDept: "CS1" },
        { id: "7", shiftId: "ca-toi-3", date: 21, employeeCode: "NV000041", employeeName: "Minh Trúc", employeeDept: "CS3" },
        { id: "8", shiftId: "ca-toi-3", date: 22, employeeCode: "NV000051", employeeName: "Nguyễn Uyên", employeeDept: "CS1" },
        { id: "9", shiftId: "ca-toi-3", date: 24, employeeCode: "NV001", employeeName: "Thanh Tâm", employeeDept: "CS1" },
        { id: "10", shiftId: "ca-toi-3", date: 25, employeeCode: "NV000037", employeeName: "Lan Nhi", employeeDept: "CS1" },
        // Ca đêm 3
        { id: "11", shiftId: "ca-dem-3", date: 20, employeeCode: "NV000023", employeeName: "Nguyễn Hữu Quốc Anh", employeeDept: "CS2" },
        { id: "12", shiftId: "ca-dem-3", date: 21, employeeCode: "NV001", employeeName: "Thanh Tâm", employeeDept: "CS1" },
        { id: "13", shiftId: "ca-dem-3", date: 22, employeeCode: "NV000023", employeeName: "Nguyễn Hữu Quốc Anh", employeeDept: "CS2" },
        { id: "14", shiftId: "ca-dem-3", date: 23, employeeCode: "NV000041", employeeName: "Minh Trúc", employeeDept: "CS3" },
        { id: "15", shiftId: "ca-dem-3", date: 25, employeeCode: "NV000023", employeeName: "Nguyễn Hữu Quốc Anh", employeeDept: "CS2" },
        // Ca sáng 3
        { id: "16", shiftId: "ca-sang-3", date: 20, employeeCode: "NV000039", employeeName: "Diễm Quỳnh", employeeDept: "CS1" },
        { id: "17", shiftId: "ca-sang-3", date: 21, employeeCode: "NV000021", employeeName: "Khánh Hoàng", employeeDept: "CS1" },
        { id: "18", shiftId: "ca-sang-3", date: 22, employeeCode: "NV000047", employeeName: "Mỹ Hà", employeeDept: "CS1" },
        { id: "19", shiftId: "ca-sang-3", date: 24, employeeCode: "NV000039", employeeName: "Diễm Quỳnh", employeeDept: "CS1" }
      ];
      const initial = initialRaw.map(a => migrateAssignmentDateKey(a as ScheduleAssignment));
      localStorage.setItem(STORAGE_KEY, JSON.stringify(initial));
      setAssignments(initial);
    } else {
      try {
        const parsed: ScheduleAssignment[] = JSON.parse(raw);
        const migrated = parsed.map(migrateAssignmentDateKey);
        setAssignments(migrated);
        if (migrated.some((a, i) => a !== parsed[i])) {
  ...
  const openModal1 = (shift: ShiftInfo, day: DayInfo) => {
    const currentOnThisCell = assignments.filter(a => a.shiftId === shift.id && a.dateKey === day.dateKey);
  ...
  const handleSaveModal1 = () => {
    ...
    let updated = assignments.filter(a => !(a.shiftId === shift.id && targetKeys.includes(a.dateKey)));
  ...
  const openModal2 = (emp: Employee, day: DayInfo) => {
    const currentOnThisCell = assignments.filter(a => a.employeeCode === emp.code && a.dateKey === day.dateKey);
  ...
  const handleSaveModal2 = () => {
    ...
    let updated = assignments.filter(a => !(targetEmpCodes.includes(a.employeeCode) && targetKeys.includes(a.dateKey)));
  ...
  if (isStaff) {
    ...
    return (
      <div className="max-w-[1200px] mx-auto p-4 sm:p-6 flex flex-col animate-in fade-in duration-200">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Chỉ hiển thị ca làm việc của <strong>{session?.name}</strong> tại chi nhánh đang chọn ở trên — đổi chi nhánh ở góc trên để xem/đăng ký ca ở cơ sở khác. Bấm vào ô trống để tự đăng ký ca làm việc.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1100px]">
            <thead>
              <tr className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                ...
                {weekDays.map((day) => (
                  <th key={day.dateKey} className="px-4 py-4 text-sm border-r border-slate-200 last:border-r-0 text-center font-normal bg-white">
            ...
            <tbody className="divide-y divide-slate-200">
              {SHIFTS.map((shift) => (
                <tr key={shift.id} className="group">
                  ...
                  {/* Day Columns */}
                  {weekDays.map((day) => {
                    const cellAssignments = assignments.filter(a => a.shiftId === shift.id && a.dateKey === day.dateKey && branchEmpCodes.has(a.employeeCode));
                    return (
                      <td
                        key={day.dateKey}
                        onClick={() => openModal1(shift, day)}
                        className="px-2.5 py-2.5 align-top border-r border-slate-200 last:border-r-0 min-w-[140px] cursor-pointer hover:bg-blue-50/30 transition-colors relative group/cell"
                      >
                        <div className="flex flex-col gap-1.5 h-full min-h-[70px]">
                          {cellAssignments.map((assign) => (
                            ...
                            >
                              <span className="truncate">{assign.employeeName}</span>
                              <button 
              ...
              {displayEmployees.map((emp) => {
                ...
                return (
                  <tr key={emp.code} className="group hover:bg-slate-50/60 transition-colors">
                    ...
                    {/* Day Columns */}
                    {weekDays.map((day) => {
                      const cellAssignments = assignments.filter(a => (a.employeeCode === emp.code || a.employeeCode === emp.id) && a.dateKey === day.dateKey);
                      return (
                        <td
                          key={day.dateKey}
