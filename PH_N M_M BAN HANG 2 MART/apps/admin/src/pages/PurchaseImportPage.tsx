...
import { Search, Plus, Download, Upload, Filter, Building2, Truck, CheckCircle2, Clock, Trash2, DollarSign, ChevronDown, ChevronUp, Eye, FileText, User, ShieldCheck, Lock, Calendar, History } from "lucide-react";
import { Button, useCatalogPresenter } from "@2mart/ui";
import { useCurrentBranch } from "../hooks/useCurrentBranch";
import { useSession } from "../hooks/useSession";
import { useEmployees } from "../hooks/useEmployees";
import { addStockAfterImport, approvePurchaseOrder, getPurchaseOrders, getProductStockAtBranch, resolveDiscountAmount, calcLineAmount, type PurchaseOrder, type PurchaseOrderItem, type DiscountType } from "../utils/branchStock";
...
export function PurchaseImportPage() {
  ...
  const [selectedSupplierCode, setSelectedSupplierCode] = useState(suppliersData[0]?.code || "");
  ...
  const currentSupplier = useMemo(() => {
    return suppliersData.find(s => s.code === selectedSupplierCode) || suppliersData[0];
  ...
  const updateCartItem = (sku: string, field: "quantity" | "costPrice" | "discountValue", val: number) => {
  ...
  const handleCreateOrder = (isDraft: boolean) => {
    ...
    const newOrder = addStockAfterImport(
      importBranch,
  ...
  const handleApproveOrder = (orderId: string, orderCode: string) => {
    if (!isManager) {
    ...
    const updated = approvePurchaseOrder(
      orderId,
      currentEmployee.name || currentEmployee.username,
    ...
    if (updated) {
      setOrders(getPurchaseOrders());
      alert(`🛡️ ĐÃ DUYỆT PHIẾU [${orderCode}] THÀNH CÔNG!\nQuản lý [${currentEmployee.name}] đã xác nhận nhập kho. Số liệu tồn kho đã được cộng dồn chính xác.`);
    }
  };

  return (
    <div className="max-w-[1600px] mx-auto p-4 sm:p-6 flex flex-col h-full relative animate-in fade-in duration-200 space-y-6">
          ...
          <table className="w-full text-left border-collapse min-w-[1050px]">
            <thead>
              <tr className="bg-slate-50 text-slate-600 text-xs uppercase tracking-wider font-bold border-b border-slate-200 sticky top-0 z-10">
                ...
                <th className="px-4 py-3.5">Người Lập / Người Nhập</th>
                <th className="px-4 py-3.5">Chi nhánh nhập</th>
                <th className="px-4 py-3.5">Hàng hóa nhập</th>
                ...
                filteredOrders.map((order) => (
                  <Fragment key={order.id}>
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-right font-black text-slate-900">{formatCurrency(order.totalAmount)}</td>
                    ...
                    {expandedRow === order.id && (
                      <tr className="bg-slate-50/90">
                        <td colSpan={9} className="p-6 border-b-2 border-blue-200 shadow-inner">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl max-w-6xl w-full shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[94vh]">
            ...
            <div className="p-5 border-b border-slate-200 flex items-center justify-between bg-blue-600 text-white">
              ...
              <button onClick={() => setIsCreateModalOpen(false)} className="w-8 h-8 rounded-full bg-blue-700 hover:bg-blue-800 text-white flex items-center justify-center font-bold transition-colors">✕</button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 flex-1 overflow-hidden">
              ...
              <div className="lg:col-span-8 p-6 flex flex-col border-r border-slate-200 overflow-y-auto space-y-5">
                ...
                {/* Cart Items Table */}
                <div className="border border-slate-200 rounded-xl overflow-hidden flex-1 flex flex-col shadow-2xs">
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-slate-600 text-xs font-bold uppercase border-b border-slate-200">
                      <tr>
                        ...
                        <th className="p-3 text-right w-36">Đơn giá nhập</th>
                        <th className="p-3 text-center w-40">Chiết khấu</th>
