...
import { InsightPanelWidget } from "../components/widgets/InsightPanelWidget";
...
export type TimeFrameType = "today" | "week" | "month" | "quarter" | "year" | "custom";

/** Tỷ suất dùng để ước tính lợi nhuận — đơn hàng không lưu giá vốn từng mặt hàng nên chỉ ước lượng */
const GROSS_MARGIN = 0.303;
const NET_MARGIN = 0.204;
...
export function DashboardPage() {
  ...
  const chartData = useMemo(() => {
    ...
    if (timeFrame === "today") {
      for (let h = 0; h <= 23; h++) buckets.set(`h${h}`, { label: `${String(h).padStart(2, "0")}:00`, sort: h, revenue: 0 });
    }
    ordersInPeriod.forEach(o => {
      const b = bucketOf(new Date(o.createdAtMs));
      const cur = buckets.get(b.key) || { label: b.label, sort: b.sort, revenue: 0 };
      cur.revenue += o.total;
    ...
    return Array.from(buckets.values())
      ...
      .map(b => ({
        name: b.label,
        revenue: b.revenue,
        grossProfit: Math.round(b.revenue * GROSS_MARGIN),
        netProfit: Math.round(b.revenue * NET_MARGIN)
      }));
  }, [ordersInPeriod, timeFrame]);

  // Số liệu THẬT tính từ đơn hàng đã bán trong kỳ
  const metrics = useMemo(() => {
    ...
    return {
      ...
      grossProfit: money(Math.round(revenue * GROSS_MARGIN)),
      grossTrend: `Ước tính (tỷ suất gộp ${(GROSS_MARGIN * 100).toFixed(1)}%)`,
      netProfit: money(Math.round(revenue * NET_MARGIN)),
      netTrend: `Ước tính (tỷ suất ròng ${(NET_MARGIN * 100).toFixed(1)}%)`
    };
  }, [ordersInPeriod, periodRange, isStaff, currentBranch]);
  ...
  const dynamicRegistry: Record<string, DashboardWidget> = useMemo(() => {
    ...
    return {
      ...
      // ĐÃ THAY THẾ CHUẨN XÁC: HIỂN THỊ CẢ LỢI NHUẬN GỘP VÀ LỢI NHUẬN RÒNG
      "kpi.gross_profit": {
        id: "kpi.gross_profit", title: "Lợi nhuận gộp", permission: "view_profit", queryKey: ["kpi", timeFrame], refreshInterval: 60000,
        component: () => <KpiCardWidget title="Doanh thu thuần trừ giá vốn" value={metrics.grossProfit} trend="up" trendLabel={metrics.grossTrend} />
      },
      "kpi.net_profit": {
        id: "kpi.net_profit", title: "Lợi nhuận ròng", permission: "view_profit", queryKey: ["kpi", timeFrame], refreshInterval: 60000,
        component: () => <KpiCardWidget title="LN gộp trừ lương & chi phí" value={metrics.netProfit} trend="up" trendLabel={metrics.netTrend} />
      },
      "chart.revenue": {
      ...
      "insight.ai": {
        id: "insight.ai", title: "AI Insight & Alerts", permission: "view_insights", queryKey: ["insight"], refreshInterval: 60000,
        component: InsightPanelWidget
      },
      "ranking.products": {
  ...
  // Ẩn lợi nhuận gộp/ròng, biểu đồ lợi nhuận, AI Insight và Top hàng bán chạy — đều là số liệu
  ...
  const layout: DashboardLayout = useMemo(() => ({
    ...
    widgets: isStaff
      ...
      : [
          { id: "kpi.revenue", position: { x: 0, y: 0, w: 3, h: 1 } },
          { id: "kpi.orders", position: { x: 3, y: 0, w: 3, h: 1 } },
          { id: "kpi.gross_profit", position: { x: 6, y: 0, w: 3, h: 1 } },
          { id: "kpi.net_profit", position: { x: 9, y: 0, w: 3, h: 1 } },
          { id: "chart.revenue", position: { x: 0, y: 1, w: 8, h: 4 } },
          { id: "insight.ai", position: { x: 8, y: 1, w: 4, h: 4 } },
          { id: "ranking.products", position: { x: 0, y: 5, w: 12, h: 3 } }
        ]
  }), [isStaff]);

  return (
    <div className="max-w-[1650px] mx-auto p-4 sm:p-6 space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row xl:items-center justify-between gap-4 sticky top-0 z-20">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-slate-800 flex items-center gap-2.5">
            ...
            {isStaff ? "Tổng Quan Bán Hàng Của Tôi" : "Tổng Quan Hệ Thống & Lợi Nhuận"}
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Đang xem dữ liệu báo cáo: <strong className="text-blue-700 font-bold">{metrics.label}</strong>
