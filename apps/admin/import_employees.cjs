...
async function run() {
  ...
  // Use range: 1 to skip the first header row from KiotViet export
  ...
  for (let i = 0; i < json.length; i++) {
    ...
    // Check if there is an explicit username column
    const existingUsername = p["Tài khoản đăng nhập"] || p["TÀI KHOẢN ĐĂNG NHẬP"] || "";
    
    const code = String(p["Mã nhân viên"] || p["Mã NV"] || p["MÃ NV"] || p["Mã hàng"] || `NV${Date.now() + i}`);
    const rawUser = String(name);
    
    let cleanUser = existingUsername;
    if (!cleanUser) {
        cleanUser = removeVietnameseTones(rawUser).replace(/\s+/g, '');
    }
    ...
    validEmployees.push({
      ...
      email: "",
      phone: String(p["Số điện thoại"] || p["Liên hệ"] || p["LIÊN HỆ"] || p["Điện thoại"] || ""),
