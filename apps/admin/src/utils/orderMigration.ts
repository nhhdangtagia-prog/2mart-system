 ...
 */
export async function deleteOrderAndRestoreStock(
  orderCode: string,
  restoreStock: (items: { sku: string; quantity: number }[], branch: string) => void
): Promise<OrderReadModelDTO | null> {
  try {
    // Delete from API first (this will also restore stock in Postgres)
    const res = await fetch(`/api/orders/${orderCode}`, { method: 'DELETE' });
    if (!res.ok) {
      // If not found in API, fallback to localStorage deletion for offline/legacy orders
      const raw = localStorage.getItem(STORAGE_KEY_ORDERS);
      if (!raw) return null;
    ...
    // Also remove from localStorage if it happens to be there
    const raw = localStorage.getItem(STORAGE_KEY_ORDERS);
    if (raw) {
      try {
        const orders = JSON.parse(raw);
        const filtered = orders.filter((o: any) => o.code !== orderCode && o.id !== orderCode);
