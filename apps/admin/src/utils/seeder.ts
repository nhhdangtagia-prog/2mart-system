  ...
  try {
    ...
    if (force || !existingCatalog || JSON.parse(existingCatalog).length < 5) {
      ...
      const topProducts: CatalogReadModelDTO[] = [
        { id: "HEI-24", sku: "HEI-24", name: "Bia Heineken Sleek Thùng 24 Lon", brandName: "Heineken", categoryName: "Đồ uống có cồn", retailPrice: 480000, costPrice: 380000, stock: 120, status: "ACTIVE", imageUrl: null },
        { id: "VNM-180", sku: "VNM-180", name: "Sữa Tươi Tiệt Trùng Vinamilk 180ml (Thùng 48 hộp)", brandName: "Vinamilk", categoryName: "Sữa & Sản phẩm từ sữa", retailPrice: 310000, costPrice: 240000, stock: 450, status: "ACTIVE", imageUrl: null },
        { id: "SIM-5L", sku: "SIM-5L", name: "Dầu Ăn Simply Nành 5 Lít", brandName: "Simply", categoryName: "Gia vị & Dầu ăn", retailPrice: 245000, costPrice: 210000, stock: 85, status: "ACTIVE", imageUrl: null },
        { id: "HAO-24", sku: "HAO-24", name: "Thùng Mì Hảo Hảo Tôm Chua Cay (30 gói)", brandName: "Acecook", categoryName: "Thực phẩm khô", retailPrice: 115000, costPrice: 90000, stock: 320, status: "ACTIVE", imageUrl: null },
        { id: "OMO-36", sku: "OMO-36", name: "Nước Giặt OMO Matic Túi 3.6Kg", brandName: "Unilever", categoryName: "Hóa phẩm & Tẩy rửa", retailPrice: 210000, costPrice: 160000, stock: 64, status: "ACTIVE", imageUrl: null }
      ];
    ...
    if (force || !existingOrders || JSON.parse(existingOrders).length < 5) {
      // Lấy danh sách sản phẩm để tra cứu giá vốn
      const catalog = JSON.parse(localStorage.getItem("kiot_rm_catalog") || "[]") as CatalogReadModelDTO[];
      const costMap = new Map<string, number>();
      catalog.forEach(p => costMap.set(p.sku, p.costPrice || Math.round(p.retailPrice * 0.7)));
      ...
      const realOrders: OrderReadModelDTO[] = (invoicesData as any[]).slice(0, 100).map((inv, idx) => {
        ...
        const items = Array.isArray(inv.items)
          ? inv.items.map((it: any) => {
              ...
              return {
                ...
                price: p,
                costPrice: costMap.get(sku) || Math.round(p * 0.7)
              };
            })
          : [];
        ...
        return {
          ...
          items,
          status: "COMPLETED",
          itemsCount: items.reduce((s: number, it: any) => s + (Number(it.quantity) || 0), 0) || Math.max(1, Math.floor((Number(inv.total) || 50000) / 50000))
        };
      });
