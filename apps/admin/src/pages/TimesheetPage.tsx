...
import { Calendar as CalendarIcon, Clock, Filter, Plus, Search, CheckCircle2, AlertCircle, RefreshCw, Eye, Tag, X, User, Users, ChevronLeft, ChevronRight, ChevronDown, Check, ShieldCheck, Play, Square, Building2, History, Award, AlertTriangle } from "lucide-react";
import { Button } from "@2mart/ui";
import { useEmployees, type Employee } from "../hooks/useEmployees";
import { useCurrentBranch } from "../hooks/useCurrentBranch";
import { useSession } from "../hooks/useSession";
import { normalizeName } from "../utils/payrollService";
...
export function TimesheetPage() {
      ...
      .then(data => {
        if (!data || data.length === 0) {
          ...
          try {
            ...
            if (changed) {
              fetch('/api/timesheets/sync', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(Object.values(migrated))
              });
            }
          } catch (e) { setRecords({}); }
        }
  ...
  const saveRecords = (newMap: Record<string, AttendanceRecord>) => {
    localStorage.setItem("kiot_rm_timesheet_v2", JSON.stringify(newMap));
    fetch('/api/timesheets/sync', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(Object.values(newMap))
    }).then(() => {
      setRecords(newMap);
      window.dispatchEvent(new Event("kiot_timesheet_change"));
    });
  };
  ...
  const displayAssignments = useMemo(() => {
    return assignments.filter(a => {
      ...
      // Filter by selected branch in ALL modes
  ...
  const handleConfirmAddShift = () => {
    ...
    fetch('/api/schedules/sync', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(updatedAssigns)
    });
          ...
          <thead>
            <tr className="bg-slate-50/90 border-b border-slate-200 sticky top-0 z-10 shadow-2xs">
              ...
              {weekDays.map(day => {
                ...
                return (
                <th key={day.dateKey} className={`px-3 py-3 text-center border-r border-slate-200 last:border-r-0 min-w-[120px] ${isToday ? 'bg-blue-50 border-t-4 border-t-blue-600 shadow-sm' : ''}`}>
                  <div className={`text-[11px] font-bold uppercase tracking-wider ${isToday ? 'text-blue-700' : 'text-slate-400'}`}>{day.name}</div>
                  <div className={`text-base font-black mt-0.5 inline-flex items-center justify-center w-8 h-8 rounded-full ${isToday ? "bg-blue-600 text-white ring-4 ring-blue-100 shadow-md" : "text-slate-800"}`}>
                    {day.date}
                  </div>
                </th>
              )})}
              ...
              return (
                <tr key={shift.id} className="group/row divide-y divide-slate-100">
                      ...
                      {uniqueEmpCodes.length === 0 ? (
                        ...
                        uniqueEmpCodes.map(code => (
                          <div key={code} className="flex items-center justify-between text-xs font-bold text-slate-700 py-1 border-t border-slate-100 first:border-t-0">
                            <span className="truncate" title={getUsername(code, empMap.get(code) || "")}>
                              {getUsername(code, empMap.get(code) || (viewMode === "self" ? branchEmployees.find(e => e.code === code)?.name : code) || "")}
                            </span>
                  ...
                  {weekDays.map(day => {
                    ...
                    // Cảnh báo trùng lặp: Nếu có > 1 nhân viên đã chấm công VÀO trong cùng ca này
                    const checkedInCount = dayAssigns.filter(a => {
                      const recId = recordId(shift.id, day.dateKey, a.employeeCode);
                      return records[recId] && records[recId].checkIn;
                    }).length;
                    const hasOverlap = checkedInCount > 1;

                    return (
                      <td key={day.dateKey} onClick={() => openAddShiftModal(shift, day)} className={`px-2 py-3 align-top border-r border-slate-200 last:border-r-0 min-w-[120px] transition-colors cursor-pointer relative ${day.dateKey === todayKey ? 'bg-blue-50/50 hover:bg-blue-100/50 border-l border-r border-blue-100' : 'bg-white hover:bg-blue-50/20'}`}>
                        {hasOverlap && (
                          <div className="absolute top-2 right-2 text-red-500 animate-pulse" title="Cảnh báo: Trùng lặp ca làm việc! Đang có nhiều hơn 1 nhân viên chấm công vào cùng một ca.">
                          ...
                          {uniqueEmpCodes.map(code => {
                            ...
                            if (!assign) {
                              ...
                              return (
                                <div
                                  key={code}
                                  onClick={(e) => { e.stopPropagation(); openAddShiftModal(shift, day, code, empName); }}
                            ...
                            return (
                              <div
                                key={code}
                                onClick={(e) => { e.stopPropagation(); openAttendanceModal(assign, shift, day); }}
                              ...
                              >
                                <div className="flex items-center gap-1.5 truncate">
                                  ...
                                  <span className="text-xs font-bold text-slate-800 group-hover/cell:text-blue-900 truncate">
                                    {getUsername(assign.employeeCode, assign.employeeName).split(' ')[0]}
                                  </span>
                                </div>
            ...
            <tbody className="divide-y divide-slate-200 text-sm">
              {SHIFTS.map(shift => {
                ...
                return empCodes.map((code, idx) => {
                  const empName = getUsername(code, empMap.get(code) || branchEmployees.find(e => e.code === code)?.name || code);
                  return (
                    <tr key={`${shift.id}-${code}`} className="hover:bg-slate-50/40 transition-colors">
                      ...
                      {monthDays.map(day => {
                        ...
                        // Cảnh báo trùng lặp ở chế độ tháng
                        const dayAssigns = shiftAssigns.filter(a => a.dateKey === day.dateKey);
                        const checkedInCount = dayAssigns.filter(a => {
                          const rId = recordId(shift.id, day.dateKey, a.employeeCode);
                          return records[rId] && records[rId].checkIn;
                        }).length;
                        const hasOverlap = checkedInCount > 1;
                        ...
                        return (
                          <td
                            ...
                            className={`relative px-1 py-2.5 text-center border-r border-slate-100 last:border-r-0 cursor-pointer transition-colors ${assign ? "hover:bg-blue-50" : "hover:bg-slate-100/70"} ${hasOverlap && rec?.checkIn ? "bg-red-50/50 hover:bg-red-100/50" : ""}`}
                            title={assign
                              ? `Chấm công ${shift.name} cho ${empName} (${day.name}, ${day.fullDate})${hasOverlap ? '\n⚠️ Cảnh báo: Đang có nhiều nhân viên chấm công vào ca này!' : ''}`
                              : `Đặt lịch bổ sung ${shift.name} cho ${empName} (${day.name}, ${day.fullDate})`}
                          >
                            {assign && renderStatusDot(rec?.status || "chua_cham")}
                            {hasOverlap && rec?.checkIn && (
            ...
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
                  <span className="font-bold text-slate-700">{getUsername(activeCell.assign.employeeCode, activeCell.assign.employeeName)}</span>
                  <span>|</span>
