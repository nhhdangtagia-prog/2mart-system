...
export function EmployeeListPage() {
  ...
  const { employees, isLoading, addEmployee, updateEmployee, deleteEmployee, deleteEmployees, reload } = useEmployees();
  ...
  const handleOpenAdd = () => {
    ...
    setFormData({
      ...
      role: "Nhân viên bán hàng",
      accessLevel: "staff",
      department: currentBranch === "379b Tôn Đức Thắng" ? "Chi nhánh 1" : "Chi nhánh 2",
      branch: currentBranch === "Tất cả chi nhánh" ? "379b Tôn Đức Thắng" : currentBranch,
      status: "Đang làm việc",
      joinDate: new Date().toISOString().split("T")[0],
      bankName: "",
      bankAccountNumber: "",
  ...
  const handleOpenEdit = (emp: Employee, e?: React.MouseEvent) => {
    ...
    setFormData({
      ...
      salaryPolicy: emp.salaryPolicy || "Mức 1"
    });
    setIsModalOpen(true);
  };
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const json = XLSX.utils.sheet_to_json<any>(worksheet, { range: 1, defval: "" });
    ...
    <div className="w-full p-4 sm:p-6 flex flex-col h-full relative animate-in fade-in duration-200">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2.5">
          ...
          <label className="flex items-center gap-2 bg-white text-slate-700 hover:bg-slate-50 shadow-sm border border-slate-200 rounded-md px-4 h-10 font-medium text-sm cursor-pointer transition-colors"><Upload className="w-4 h-4" /> Import<input type="file" accept=".xlsx,.xls" className="hidden" onChange={handleFileUpload} /></label>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        ...
                        {emp.email && (
                          <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-0.5 truncate w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select 
                    ...
                    onChange={(e) => setFormData({ ...formData, salaryPolicy: e.target.value })}
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-800 transition-all"
                  >
                    <option value="Mức 1">Mức 1 (18k/h thường, 20k/h đêm)</option>
                    <option value="Mức 2">Mức 2 (20k/h thường, 22k/h đêm)</option>
                  </select>
                </div>
